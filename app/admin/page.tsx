...
import { differenceInDays, differenceInMonths, differenceInWeeks, eachDayOfInterval, eachMonthOfInterval, eachWeekOfInterval, endOfWeek, interval, max, min, startOfDay, startOfWeek } from 'date-fns'
...
const getSalesData = async (createdAfter: Date | null, createdBefore: Date | null) => {
    ...
    const { array, format } = getChartDateArray(createdAfter || startOfDay(chartData[0].createdAt), createdBefore || new Date())

    const dayArray = array.map((date) => {
        return { date: format(date), totalSales: 0 }
    })
...
const getUserData = async (createdAfter: Date | null, createdBefore: Date | null) => {
    ...
    const dayArray = array.map((date) => {
        return { date: format(date), totalUsers: 0 }
    })
...
const getProductData = async (createdAfter: Date | null, createdBefore: Date | null) => {
    const createdAtQuery: Prisma.OrderWhereInput['createdAt'] = {}
    if (createdAfter) createdAtQuery.gte = createdAfter
    if (createdBefore) createdAtQuery.lte = createdBefore
    ...
    return {
        chartData: chartData.map((product) => ({
            name: product.name,
            revenue: product.orders.reduce((total, order) => total + order.pricePaidInCents / 100, 0)
...
const AdminDashboardPage = async (
    ...
    const totalSalesRangeOption = getRangeOption(totalSalesRange, totalSalesRangeFrom, totalSalesRangeTo) || RANGE_OPTIONS.last_7_days
    const newCustomersRangeOption = getRangeOption(newCustomersRange, newCustomersRangeFrom, newCustomersRangeTo) || RANGE_OPTIONS.last_7_days
    const revenueByProductRangeOption = getRangeOption(revenueByProductRange, revenueByProductRangeFrom, revenueByProductRangeTo) || RANGE_OPTIONS.all_time
    ...
    return (
        <div className='space-y-8'>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
                <ChartCard title="Total Sales" queryKey='totalSalesRange' selectedRangeLabel={totalSalesRangeOption.label}>
                    <OrdersByDayChart data={salesData.chartData} />
                </ChartCard>
                <ChartCard title="New Customers" queryKey='newCustomersRange' selectedRangeLabel={newCustomersRangeOption.label} >
                    <UsersByDayChart data={userData.chartData} />
                </ChartCard>
                <ChartCard title="Revenue by Product" queryKey='revenueByProductRange' selectedRangeLabel={revenueByProductRangeOption.label}>
...
const getChartDateArray = (startDate: Date, endDate: Date = new Date()) => {
    ...
    if (months < 30) {
        return {
            ...
            format: new Intl.DateTimeFormat('en', { month: 'long', year: 'numeric' }).format
        }
    }
