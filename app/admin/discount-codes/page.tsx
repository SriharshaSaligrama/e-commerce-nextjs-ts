...
const getUnexpiredDiscountCodes = () => {
    return db.discountCode.findMany({
        select: SELECT_FIELDS,
...
const DiscountCodesPage = async () => {
    const [expiredDiscountCodes, unexpiredDiscountCodes] = await Promise.all([getExpiredDiscountCodes(), getUnexpiredDiscountCodes()])
    return (
        <>
...
const DiscountCodesTable = ({ discountCodes, isInactive = false, canDeactivate = false }: DiscountCodesTableProps) => {
    return <Table>
        <TableHeader>
            <TableRow>
