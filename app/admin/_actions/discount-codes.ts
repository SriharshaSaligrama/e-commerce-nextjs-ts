...
const addSchema = z.object({
    ...
    message: 'Cannot select products if all products is selected',
    path: ['productIds'],
}).refine(data => data.allProducts || data.productIds != null, {
    message: 'Must select products if all products is not selected',
    path: ['productIds'],
})

export const addDiscountCode = async (prevState: unknown, formData: FormData) => {
...
export const toggleDiscountCodeActive = async (id: string, isActive: boolean) => {
